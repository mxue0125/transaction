...
def load_csv_to_staging(cursor, filepath, source_file):
    ...
    with open(filepath, newline='', encoding='utf-8-sig') as f:
        ...
        for row in reader:
            ...
            cursor.execute("""
                INSERT INTO fd_transaction_staging (
...
def insert_staging_to_final(cursor, source_file):
    cursor.execute("""
        ...
        SELECT
            ...
            Type, Price, Quantity, Commission, Fees, Accrued_Interest, Amount,
            CASE 
                WHEN Settlement_Date_Raw REGEXP '^[0-9]{2}/[0-9]{2}/[0-9]{4}$' 
                THEN STR_TO_DATE(Settlement_Date_Raw, '%m/%d/%Y')
                ELSE NULL
            END,
            Source_File
        FROM fd_transaction_staging
        WHERE Run_Date_Raw REGEXP '^[0-9]{2}/[0-9]{2}/[0-9]{4}$'
          AND Source_File = %s
    """, (source_file,))
...
def process_file(filepath, conn):
    ...
    try:
        if is_file_loaded(cursor, source_file, 'fd_transaction_staging'):
            print(f"[SKIP] {source_file} already in staging.")
            return
        if is_file_loaded(cursor, source_file, 'fd_transaction'):
            print(f"[SKIP] {source_file} already in fd_transaction.")
            return

        print(f"[LOAD] Loading {source_file} into staging...")
        rows_inserted, rows_skipped = load_csv_to_staging(cursor, filepath, source_file)
        conn.commit()
        print(f"       Staging: {rows_inserted} inserted, {rows_skipped} skipped.")
